...
st.header("Documents & Files Sentimental Analysis")
st.write("Easily Analyze **PDF Legal Contract Documents, Long Text Messages, Lecture Notes etc. for Sentiments **Positivity, negativity or neutrality statements.**")
...
st.header(f" Text Sentimental Analysis ")
...
def process_data(): 
 ...
 if replicate_api =='':
    ...
    text_prompt_data_process = "Sentiments Analysis for this text '" + data_regex + "'  "
    ...
    st.success(f" **Arctic AI Sentimental Analysis Result:** {st.session_state.event_result} ")
    ...
    <script language="javascript">
          alert('Arctic AI Sentimental Analysis Successful. Please Scroll Up the page to view Result...');
    </script>
...
st.button('Run Text Sentimental Analysis', on_click=process_data)
...
def process_data_docs(text_prompt_data_docs): 
 ...
 if replicate_api =='':
    ...
    text_prompt_data_process = "Sentiments Analysis for this text '" + data_regex_docs + "'  "
    os.environ['REPLICATE_API_TOKEN'] = replicate_api  
    # with st.spinner("please wait....."):      
    query = []
    for event in replicate.stream(
    ...
    for res in query:
     ...
     # result_query = "\n".join(query)
    st.session_state.event_result = result_query
    st.success(f" **Arctic AI Sentimental Analysis Response:** {st.session_state.event_result} ")
    # Update the session_state with the event result
    st.session_state.event_result = result_query
    html_string = f'''
...
st.header(f" PDF Documents & Files Sentiments Analysis ")


uploaded_file = st.file_uploader("Choose a pdf file", "pdf")
if uploaded_file is not None:
    fx = extract_text(uploaded_file)
    st.success("PDF File Successfully Extracted. You Can Now Now Sentiments Analysis")
     # Update the session_state with the event result
    ...
    st.button('Run Sentiments Analysi Of PDF Documents', on_click=process_data_docs, args=[text_prompt_data_docs])
    st.button('Clear AI Response.', on_click=clear_response_history_docs)
