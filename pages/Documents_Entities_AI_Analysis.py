...
st.header("Documents & Files Entity Analysis")
st.write("Easily Analyze **PDF Legal Contract Documents, Long Text Messages, Lecture Notes etc.** and list all **People, Persons, Organizations, Companies and all the entities.** involved in the Documents")
...
st.header(f" Text Entities Analysis ")
...
def process_data(): 
 ...
 if replicate_api =='':
    ...
    st.success(f" **Arctic AI Entity Analysis Result:** {st.session_state.event_result} ")
    ...
    <script language="javascript">
          alert('Arctic AI Entity Analysis Successful. Please Scroll Up the page to view Result..');
    </script>
...
st.button('Run Text Entity Analysis', on_click=process_data)
...
def process_data_docs(text_prompt_data_docs): 
 ...
 if replicate_api =='':
    ...
    for res in query:
     ...
     # result_query = "\n".join(query)
    st.session_state.event_result = result_query
    st.success(f" **Arctic AI Entities Analysis Response:** {st.session_state.event_result} ")
    # Update the session_state with the event result
    st.session_state.event_result = result_query
    html_string = f'''
        
    <script language="javascript">
          alert('Arctic AI Entities Analysis Successful. Please Scroll Up the page to view Result...');
    </script>
...
st.header(f" PDF Documents & Files Entities Analysis ")


uploaded_file = st.file_uploader("Choose a pdf file", "pdf")
if uploaded_file is not None:
    fx = extract_text(uploaded_file)
    st.success("PDF File Successfully Extracted. You Can Now Now Entities Analysis")
     # Update the session_state with the event result
    ...
    st.button('Run Entities Analysis Of PDF Documents', on_click=process_data_docs, args=[text_prompt_data_docs])
    st.button('Clear AI Response.', on_click=clear_response_history_docs)
