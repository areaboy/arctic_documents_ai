...
st.header("Documents & Files Keywords Analysis")
st.write("Easily Analyze **PDF Legal Contract Documents, Long Text Messages, Lecture Notes etc.** and list all **keywords and Keyphrases.** involved in the Documents")
...
st.header(f" Text Keywords Analysis ")
...
def process_data(): 
 ...
 if replicate_api =='':
    ...
    text_prompt_data_process = "list all the keywords in  this text seperated by commas '" + data_regex + "'  "
    ...
    st.success(f" **Arctic AI Keywords Analysis Result:** {st.session_state.event_result} ")
    ...
    <style>
          .data_css{{
          background:#ddd;color:black;padding:2px;border:none;border-radius:25%;
          margin: 0.2%;
...
st.button('Run Text Keywords Analysis', on_click=process_data)
...
def process_data_docs(text_prompt_data_docs): 
 ...
 if replicate_api =='':
    ...
    text_prompt_data_process = " list all the keywords in  this text seperated by commas '" + data_regex_docs + "'  "
    os.environ['REPLICATE_API_TOKEN'] = replicate_api  
    # with st.spinner("please wait....."):      
    query = []
    for event in replicate.stream(
    ...
    for res in query:
     ...
     # result_query = "\n".join(query)
    st.session_state.event_result = result_query
    st.success(f" **Arctic AI Keywords Analysis Response:** {st.session_state.event_result} ")
    # Update the session_state with the event result
    st.session_state.event_result = result_query
    data_regex2 = re.sub(r'[^A-Za-z0-9, .]', '', st.session_state.event_result)
    html_string = f'''
...
st.header(f" PDF Documents & Files Keywords Analysis ")


uploaded_file = st.file_uploader("Choose a pdf file", "pdf")
if uploaded_file is not None:
    fx = extract_text(uploaded_file)
    st.success("PDF File Successfully Extracted. You Can Now Now Keywords Analysis")
     # Update the session_state with the event result
    ...
    st.button('Run Keywords Analysis Of PDF Documents', on_click=process_data_docs, args=[text_prompt_data_docs])
    st.button('Clear AI Response.', on_click=clear_response_history_docs)
